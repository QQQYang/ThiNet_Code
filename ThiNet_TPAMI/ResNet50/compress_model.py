...
def get_index(compress_rate, compress_layer, gpu, compress_block):
    ...
    if len(compress_block) > 1:
        model_dir = '-'.join([str(i) for i in compress_layer])+'_'+str(1)+'_'+str(compress_rate)
        model_def = os.path.join('/ssd/yqian/prune/model/ResNet50', model_dir, 'trainval.prototxt')
    ...
    for i in range(len(compress_layer)):
        layer_index, layer_w = [], []
        for j in range(len(compress_block)):
            single_index, single_w = value_sum(Xs[i][j], Ys[i][j], compress_rate)
...
def compress_net(index, w, compress_layer, compress_block, compress_rate):
    ...
    net_new = caffe.Net('/ssd/yqian/prune/model/ResNet50/' + '-'.join([str(i) for i in compress_layer])+'_'+'-'.join([str(i) for i in compress_block])+'_'+str(compress_rate)+'/'+'trainval.prototxt', caffe.TEST)
    if len(compress_block) > 1:
        model_dir = '-'.join([str(i) for i in compress_layer])+'_'+str(1)+'_'+str(compress_rate)
        model_def = os.path.join('/ssd/yqian/prune/model/ResNet50', model_dir, 'trainval.prototxt')
        model_weights = os.path.join('/ssd/yqian/prune/model/ResNet50', model_dir, 'snapshot/_iter_10000.caffemodel.h5')
    ...
    net_new_name = '/ssd/yqian/prune/model/ResNet50/'+'-'.join([str(i) for i in compress_layer])+'_'+'-'.join([str(i) for i in new_compress_block])+'_'+str(compress_rate)+'/'+'prune.caffemodel'
