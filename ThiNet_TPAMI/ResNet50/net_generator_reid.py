    ...
    def write(self, name=None, folder=None, deploy=False):
        ...
        with open(name, 'wb') as f:
            if deploy:
                ...
                net_str = net_str[net_str.find('top: "data"')+len('top: "data"\n}\n'):-1]
        ...
        for i in range(len(compress_layer)):
            for j in range(len(compress_block)):
                if compress_block[j] == 1:
                    rate1[0, compress_layer[i]] = rate1[0, compress_layer[i]] * compress_rate[j]
                else:
                    rate2[0, compress_layer[i]] = rate2[0, compress_layer[i]] * compress_rate[j]
...
def solver_and_prototxt(root_dir, compress_layer, compress_rate, compress_block):
    ...
    name = '-'.join([str(i) for i in compress_layer])+'_'+'-'.join([str(i) for i in compress_block])+'_'+'-'.join([str(i) for i in compress_rate])
    builder = Net(name + '-deploy')
