...
from net_generator_reid import solver_and_prototxt2
import random
import time
import os
import argparse
...
def get_index(compress_rate, compress_layer, gpu, compress_block, method):
    ...
    if len(compress_block) > 1:
        ...
        model_def = os.path.join('/ssd/yqian/prune/model/reid', model_dir, 'deploy.prototxt')
        model_weights = os.path.join('/ssd/yqian/prune/model/reid', model_dir, 'snapshot/_iter_10000.caffemodel.h5')
    ...
    # set parameters
    sample_num = 10000  # 1000 categories sample sample_num images
            ...
            for i in range(len(compress_layer)):
                for j in range(len(compress_block)):
                    if compress_block[j] == 1:
                        input_layer = 'res' + layers[compress_layer[i]] + '_branch2a'
                        output_layer = 'res' + layers[compress_layer[i]] + '_branch2b'
                        kernel_size = 3
                        padding = 1
                    else:
                        input_layer = 'res' + layers[compress_layer[i]] + '_branch2b'
                    ...
                    if len(Xs) < i+1:
                        X = np.zeros([channel_num * sample_num, Filters.shape[1]])
                        Y = np.zeros([channel_num * sample_num, 1])
                        Xs.append([X])
                        Ys.append([Y])
                        count.append([0])
                    elif len(Xs[i]) < j+1:
...
def compress_net(index, w, compress_layer, compress_block, compress_rate):
    ...
    net_new = caffe.Net('/ssd/yqian/prune/model/reid/' + '-'.join([str(i) for i in compress_layer])+'_'+'-'.join([str(i) for i in compress_block])+'_'+'-'.join([str(i) for i in compress_rate])+'/'+'trainval.prototxt', caffe.TEST)
    if len(compress_block) > 1:
        model_dir = '-'.join([str(i) for i in compress_layer])+'_'+str(compress_block[0])+'_'+str(compress_rate[0])
        model_def = os.path.join('/ssd/yqian/prune/model/reid', model_dir, 'deploy.prototxt')
        model_weights = os.path.join('/ssd/yqian/prune/model/reid', model_dir, 'snapshot/_iter_10000.caffemodel.h5')
    ...
    # fc
    net_new.params['fc256'][0].data[...] = net.params['fc256'][0].data
    net_new.params['fc256'][1].data[...] = net.params['fc256'][1].data
    net_new_name = '/ssd/yqian/prune/model/reid/'+'-'.join([str(i) for i in compress_layer])+'_'+'-'.join([str(i) for i in new_compress_block])+'_'+'-'.join([str(i) for i in compress_rate])+'/'+'prune.caffemodel'
...
def pipeline(opt):
    solver_and_prototxt2('/ssd/yqian/prune/model/reid', opt.compress_layer, opt.compress_rate, opt.compress_block)
